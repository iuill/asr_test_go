...
画面の「5分ごとに自動保存」を ON にすると、録音ごとに EXE 横の `transcripts` フォルダへ日時付きのテキストファイルを作り、5分ごとと録音停止時に内容を更新します。この切替も `asr-studio.preferences.json` に保存されます。自動保存するのは確定した文字起こしだけです。「結果を消去」で画面を空にしても、録音中の自動保存ファイルに蓄積した結果は消しません。保存エラーは画面に表示します。
