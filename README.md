...
画面の「結果ごとに自動保存」を ON にすると、録音ごとに EXE 横の `transcripts` フォルダへ日時付きのテキストファイルを作り、確定した文字起こしが追加されるたびと録音停止時に内容を更新します。この切替も `asr-studio.preferences.json` に保存されます。途中の聞き取り結果は保存しません。「結果を消去」で画面を空にしても、録音中の自動保存ファイルに蓄積した結果は消しません。保存エラーは画面に表示します。
